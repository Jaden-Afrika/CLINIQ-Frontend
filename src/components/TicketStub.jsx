function TicketStub({ ticketNumber, doctorName, date, time, label = 'Your ticket', children }) {
  const formattedTime = time ? (typeof time === 'string' && time.length >= 5 ? time.slice(0, 5) : time) : null
      ...
      <div className="px-6 pt-7 pb-6 sm:px-10 sm:pt-9">
        ...
        {(doctorName || date || formattedTime) && (
          <div className="mt-7 border-t border-dashed border-ink/25 pt-5 text-sm text-ink/70">
            ...
            {formattedTime && <p className="mt-1">{formattedTime}</p>}
